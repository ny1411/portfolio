...
const basePortraitUrl = encodeURI(
	`${PUBLIC_BASE_URL}parallax-images/portfolio portrait.png`,
);
const maskedPortraitUrl = encodeURI(
	`${PUBLIC_BASE_URL}parallax-images/spiderman portfolio portrait.png`,
);
...
import {
	applyHeroVariables,
	getHeroGeometry,
} from './masking-parallax/geometry';
import { clamp, damp, length } from './masking-parallax/math';
import {
	createMotionState,
	updateMotionState,
} from './masking-parallax/motion';
import {
	createSvgCache,
	updateSvgReveal,
	type SvgCache,
} from './masking-parallax/svgReveal';
import type {
	MotionState,
	Point,
	ViewportSnapshot,
} from './masking-parallax/types';
import {
	getCenteredPortraitFrame,
	getViewportSnapshot,
} from './masking-parallax/viewport';
...
export function MaskingParallaxSection() {
	...
	const featherStdDev =
		profile.tier === 'low' ? '1.5' : profile.isLowPower ? '2.5' : '4.5';
	...
	useEffect(() => {
		const updatePointerTarget = (
			...
			pointerTargetRef.current = {
				x: clamp(
					(x - (frame.x + frame.width * 0.5)) / (frame.width * 0.5),
					-1,
					1,
				),
				y: clamp(
					(y - (frame.y + frame.height * 0.5)) / (frame.height * 0.5),
					-1,
					1,
				),
			};
		...
		const handlePointerDown = (event: PointerEvent) => {
			...
			activePointerIdRef.current = startedInsidePortrait
				? event.pointerId
				: null;
		};
	...
	useEffect(() => {
		...
		return rafScheduler.schedule((time) => {
			...
			hoverMotionRef.current = damp(
				...
				interactionTarget || !viewport.mobile
					? HOVER_LERP
					: MOBILE_INTERACTION_LERP_OUT,
			);
			...
			const revealTarget = reducedMotion
				? 0
				: hoverMotionRef.current *
	...
	return (
		...
		>
			<div className="masking-parallax-stage">
				<div
					className="masking-parallax-depth masking-parallax-depth--backdrop"
					aria-hidden="true"
				/>
				<div
					className="masking-parallax-depth--atmosphere-wrap"
					aria-hidden="true"
				>
				...
				>
					<defs>
						...
						<mask id="masking-blob-alpha" maskUnits="userSpaceOnUse">
							<path
								ref={blobFeatherPathRef}
								fill="#fff"
								opacity="0.38"
								filter="url(#masking-blob-feather)"
							/>
							<path ref={blobCorePathRef} fill="#fff" opacity="0.96" />
						</mask>
					</defs>
					<g
						className="masking-parallax-reveal-layer"
						ref={revealLayerRef}
						mask="url(#masking-blob-alpha)"
					>
						<image
						...
						/>
						<rect
							className="masking-parallax-reveal-grade"
							x="0"
							y="0"
							width="100%"
							height="100%"
