...
const githubLogo = `${PUBLIC_BASE_URL}icons/GitHub.svg`;
const linkedinLogo = `${PUBLIC_BASE_URL}icons/LinkedIn.svg`;
const whatsappLogo = `${PUBLIC_BASE_URL}icons/WhatsApp.svg`;
const XLogo = `${PUBLIC_BASE_URL}icons/X.svg`;
import { useScrollStore } from '../scrolly/scrollStore';
import { SparklesCore } from '../ui/sparkles';

const contactPage = resumeContent.resumePages.find(
	(page) => page.id === 'contact',
);
const contactLines = contactPage?.body ?? [];
const emailLine = contactLines.find((line) => line.startsWith('Email:'));
const contactEmail =
	emailLine?.replace('Email:', '').trim() ?? 'neerajyamaji@gmail.com';
...
export function ContactSection() {
	...
	useEffect(() => {
		const syncGlassOpacity = () => {
			...
			const opacity =
				activeSceneId === 'contact' ? getContactGlassOpacity(sceneProgress) : 0;

			glassPanelRef.current?.style.setProperty(
				'--contact-glass-opacity',
				opacity.toFixed(3),
			);
		};
	...
	return (
		<div className="scene-layout scene-layout--contact contact-cta-scene">
			<div className="contact-sparkles-cta">
				<span
					ref={glassPanelRef}
					className="contact-sparkles-cta__readability-panel"
					aria-hidden="true"
				/>
				<span
					className="contact-sparkles-cta__ambient-bloom"
					aria-hidden="true"
				/>
				<a
					className="contact-sparkles-cta__title"
...
function getContactGlassOpacity(progress: number): number {
	const progressInRange = Math.min(
		...
		Math.max(
			0,
			(progress - GLASS_REVEAL_START) / (GLASS_REVEAL_END - GLASS_REVEAL_START),
		),
	);
